...
def sell():
    ...
    # User reached route via POST (as by submitting a form via POST), and entered some information.
        ...
        # Update the user's cash total in the database.
        db.execute("UPDATE users SET cash = :new_cash_total WHERE id = :user_id",
                   new_cash_total=new_cash_total, user_id=user_id)
        # Record the transaction in the stocks table with negative shares to indicate sale.
        transacted = datetime.now()
        db.execute("INSERT INTO stocks (user_id, symbol, name, shares, price, transacted) VALUES (:user_id, :symbol, :name, :shares, :price, :transacted)",
                   user_id=user_id, symbol=symbol, name=name, shares=sold_shares, price=price, transacted=transacted)
        # Redirect to the index page with updated portfolio.
